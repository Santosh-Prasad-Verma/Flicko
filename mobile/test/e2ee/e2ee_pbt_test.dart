...
void main() {
  ...
  group('Phase 1: Ratchet Properties', () {
    test('R5.1, R5.2, R5.6, R5.7: Ratchet encrypt/decrypt round-trip', () async {
      ...
      for (var i = 0; i < pbtRuns; i++) {
        final plaintext = randomBytes(random.nextInt(1000) + 1);
        final ad = randomBytes(16);
    ...
    test('R5.8: Ratchet out-of-order delivery', () async {
      ...
      final m1 = await DoubleRatchet.encrypt(state: aliceState, plaintext: randomBytes(10), associatedData: Uint8List(0));
      aliceState = m1.state;
      
      final m2 = await DoubleRatchet.encrypt(state: aliceState, plaintext: randomBytes(10), associatedData: Uint8List(0));
      aliceState = m2.state;
      
      final m3 = await DoubleRatchet.encrypt(state: aliceState, plaintext: randomBytes(10), associatedData: Uint8List(0));
      aliceState = m3.state;
  ...
  group('Phase 4: Identity Verification Properties', () {
    test('R9.1: Fingerprint symmetry', () async {
      for (var i = 0; i < pbtRuns; i++) {
        final aliceIk = randomBytes(32);
        final bobIk = randomBytes(32);
