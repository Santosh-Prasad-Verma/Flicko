  ...
  Future<void> _loadLogs() async {
    ...
    try {
      ...
      if (_selectedFilter == 'Member Updates') {
        baseQuery = baseQuery.inFilter('action_type', ['ban', 'kick', 'timeout', 'mute', 'deafen', 'role_update', 'nickname_update']);
      } else if (_selectedFilter == 'Channel Updates') {
        baseQuery = baseQuery.inFilter('action_type', ['channel_create', 'channel_update', 'channel_delete']);
      } else if (_selectedFilter == 'Role Updates') {
        baseQuery = baseQuery.inFilter('action_type', ['role_create', 'role_update', 'role_delete']);
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Audit Log', style: TextStyle(color: Color(FlickoColors.textPrimary))),
        actions: [
          IconButton(
