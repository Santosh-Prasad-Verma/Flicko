...
class BotPollSettingsScreen extends ConsumerStatefulWidget {
  final String serverId;

  const BotPollSettingsScreen({
    super.key,
    required this.serverId,
  });

  @override
  ConsumerState<BotPollSettingsScreen> createState() => _BotPollSettingsScreenState();
}

class _BotPollSettingsScreenState extends ConsumerState<BotPollSettingsScreen> {
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Text(
              'Create and manage polls with multiple options for your community.',
          ...
          _buildSection('Status', [
            SwitchListTile(
              title: Text('Enable Poll Bot', style: GoogleFonts.inter(color: const Color(FlickoColors.textPrimary), fontSize: 16, fontWeight: FontWeight.w500)),
              subtitle: Text('Allow users to create polls', style: GoogleFonts.inter(color: const Color(FlickoColors.textMuted), fontSize: 12)),
          ...
          _buildSection('Settings', [
            _buildInfoField('Max Options', '${_settings?['max_options'] ?? 10}'),
            _buildInfoField('Poll Duration', '${_settings?['default_duration'] ?? 24} hours'),
            _buildInfoField('Allowed Roles', _settings?['allowed_roles'] != null ? 'Configured' : 'Everyone'),
          ]),
          const SizedBox(height: 24),
          ...
          _buildSection('Commands', [
            _buildInfoField('/poll create', 'Create a new poll'),
            _buildInfoField('/poll vote', 'Vote on a poll'),
            _buildInfoField('/poll results', 'Show poll results'),
            _buildInfoField('/poll end', 'End a poll early'),
          ]),
        ],
      ),
    );
