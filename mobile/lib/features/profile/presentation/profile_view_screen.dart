...
class _ProfileViewScreenState extends ConsumerState<ProfileViewScreen> {
  ...
  Future<void> _loadMutualServers(String currentUserId, String profileId) async {
    try {
      ...
      try {
        ...
        final myServerIds = (myMemberships as List).map((m) => m['server_id'] as String).toSet();
        ...
        if (mutualIds.isNotEmpty) {
          final servers = await _client.from('servers').select('id, name, icon').inFilter('id', mutualIds);
          setState(() {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Action failed: $e'), backgroundColor: const Color(FlickoColors.red)),
        );
      }
    } finally {
  ...
  Future<bool> _showConfirmDialog(String title, String message) async {
    return await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            title: Text(title,
                style: GoogleFonts.epilogue(color: _white, fontWeight: FontWeight.w900, fontSize: 18)),
            content: Text(message,
                style: GoogleFonts.inter(color: _white.withValues(alpha: 0.7), fontSize: 14)),
            actions: [
              TextButton(
                onPressed: () => Navigator.of(ctx).pop(false),
                child: Text('CANCEL',
                    style: GoogleFonts.spaceGrotesk(color: _muted, fontWeight: FontWeight.w700)),
              ),
              TextButton(
                onPressed: () => Navigator.of(ctx).pop(true),
                child: Text('CONFIRM',
                    style: GoogleFonts.spaceGrotesk(
                        color: const Color(FlickoColors.red), fontWeight: FontWeight.w800)),
              ),
            ],
          ),
        ) ??
          ...
          children: [
            ...
            Text('USER NOT FOUND',
                style: GoogleFonts.epilogue(
                    color: _white, fontSize: 20, fontWeight: FontWeight.w900, fontStyle: FontStyle.italic)),
            const SizedBox(height: 8),
            Text(_errorMessage ?? 'This profile is unavailable or private.',
                style: GoogleFonts.inter(color: _muted, fontSize: 13), textAlign: TextAlign.center),
            const SizedBox(height: 32),
            GestureDetector(
              onTap: () => context.pop(),
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12),
                decoration: BoxDecoration(color: _surface, border: Border.all(color: _muted.withValues(alpha: 0.3))),
                child: Text('GO BACK',
                    style: GoogleFonts.spaceGrotesk(color: _white, fontWeight: FontWeight.w800)),
              ),
            ),
          ],
  ...
  Widget _buildBanner(UserModel profile, bool isOwnProfile) {
    ...
    final accentColor = Color(int.parse(accentHex.replaceFirst('#', '0xFF')));
        ...
        children: [
          ...
          Positioned(
            ...
            child: Row(
              ...
              children: [
                _buildCircleBtn(Icons.arrow_back, () => context.pop()),
                Row(
                  children: [
                    _buildCircleBtn(Icons.share_outlined, () {
                      Clipboard.setData(ClipboardData(text: 'https://flicko.app/u/${profile.username}'));
          ...
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(color: accentColor.withValues(alpha: 0.2), blurRadius: 30, spreadRadius: 5)
                ],
              ),
              child: UserAvatar(
        ...
        Row(
          children: [
            Text('@${profile.username}',
                style: GoogleFonts.spaceMono(color: _neon, fontSize: 14, fontWeight: FontWeight.w600)),
            const SizedBox(width: 12),
            Container(width: 8, height: 8, decoration: BoxDecoration(color: statusColor, shape: BoxShape.circle)),
            const SizedBox(width: 8),
            Text(profile.onlineStatus.toUpperCase(),
                style: GoogleFonts.spaceMono(
                    color: _muted, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
          ],
        ),
    ...
    if (isOwnProfile) {
      return Row(
        children: [
          ...
          const SizedBox(width: 12),
          _buildSquareBtn(Icons.settings_rounded, () => context.push('/u/settings')),
        ],
      );
    }
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(label,
                style: GoogleFonts.spaceGrotesk(
                    color: txtColor, fontSize: 13, fontWeight: FontWeight.w900, letterSpacing: 0.5)),
          ],
        ),
      ),
    );
  ...
  Widget _buildInfoBadge(String text, IconData icon) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: _surface, border: Border.all(color: _white.withValues(alpha: 0.05))),
      child: Row(
        children: [
          Icon(icon, color: _neon, size: 18),
          const SizedBox(width: 12),
          Text(text, style: GoogleFonts.spaceGrotesk(color: _white, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
          ...
          child: Row(
            ...
            children: [
              ...
              Text(role['name']?.toString().toUpperCase() ?? 'ROLE',
                  style: GoogleFonts.spaceMono(color: color, fontSize: 10, fontWeight: FontWeight.w900)),
            ],
          ),
        );
      }).toList(),
          ...
          return Tooltip(
            ...
            child: Container(
              ...
              child: server['icon'] != null
                  ...
                  : Center(child: Text(server['name'][0].toString().toUpperCase(), 
                      style: GoogleFonts.epilogue(color: _neon, fontWeight: FontWeight.w900))),
            ),
          );
        },
      ),
    ...
    return Container(
      ...
      child: _isEditingNote
              ...
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    filled: true,
                    fillColor: _bg,
                    border: OutlineInputBorder(borderSide: BorderSide(color: _white.withValues(alpha: 0.1))),
                  ),
                ),
                const SizedBox(height: 12),
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      child: Text('SAVE', style: GoogleFonts.spaceGrotesk(fontWeight: FontWeight.w900)),
                    ),
                  ],
                ),
              ],
          ...
          : GestureDetector(
              ...
              child: Row(
                children: [
                  const Icon(Icons.sticky_note_2_rounded, color: _muted, size: 20),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Text(
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => SafeArea(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Container(margin: const EdgeInsets.only(top: 12, bottom: 8), width: 40, height: 4, color: _white.withValues(alpha: 0.1)),
            if (!isOwnProfile) ...[
  ...
  Widget _sheetItem(IconData icon, String label, VoidCallback onTap, {Color? color}) {
    return ListTile(
      leading: Icon(icon, color: color ?? _white, size: 22),
      title: Text(label, style: GoogleFonts.spaceGrotesk(color: color ?? _white, fontWeight: FontWeight.w700)),
      onTap: onTap,
    );
  }
}
