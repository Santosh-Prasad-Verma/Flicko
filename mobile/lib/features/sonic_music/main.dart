...
import 'package:mobile/features/sonic_music/Helpers/handle_native.dart';
import 'package:mobile/features/sonic_music/Helpers/import_export_playlist.dart';
...
import 'package:mobile/features/sonic_music/theme/app_theme.dart';
...
Future<void> main() async {
  ...
  if (Platform.isWindows || Platform.isLinux || Platform.isMacOS) {
    await Hive.initFlutter('Sonic/Database');
...
class _MyAppState extends State<MyApp> {
  ...
  void initState() {
    ...
    if (Platform.isAndroid || Platform.isIOS) {
      // Listen for incoming share intents (text, media, etc.) while the app is running in the background/foreground
          ...
          if (value.isNotEmpty) {
            for (final file in value) {
              if (file.type == SharedMediaType.text || file.type == SharedMediaType.url) {
                Logger.root.info('Received shared text/url on stream: ${file.path}');
      ...
      // Handle share intents when the app is launched from a cold start
      ReceiveSharingIntent.instance.getInitialMedia().then(
        (List<SharedMediaFile> value) {
          if (value.isNotEmpty) {
            for (final file in value) {
              if (file.type == SharedMediaType.text || file.type == SharedMediaType.url) {
                Logger.root.info('Received shared text/url initially: ${file.path}');
                handleSharedText(file.path, navigatorKey);
              } else if (file.path.endsWith('.json')) {
                final List playlistNames = Hive.box('settings')
                        .get('playlistNames')
