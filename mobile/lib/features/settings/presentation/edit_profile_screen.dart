...
class _EditProfileScreenState extends ConsumerState<EditProfileScreen> {
  ...
  Future<void> _pickAvatar() async {
    ...
    if (picked != null) {
      ...
      try {
        final appwriteService = AppwriteStorageService();
        if (!appwriteService.isConfigured) {
          throw Exception('Appwrite not configured');
        }

        final file = File(picked.path);
        final uploadedUrl = await appwriteService.uploadImage(file);
            ...
            image: _bannerUrl != null
                ? DecorationImage(
                    image: _bannerUrl!.startsWith('http')
                        ...
                        : FileImage(Uri.parse(_bannerUrl!).toFilePath() as dynamic) as ImageProvider,
                    fit: BoxFit.cover,
                  )
                : null,
          ),
