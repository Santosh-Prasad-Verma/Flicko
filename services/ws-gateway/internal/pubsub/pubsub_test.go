...
t.Errorf("ActiveSubscriptions = %d, want 1 (consolidated pattern subscription)", snap.ActiveSubscriptions)
}
}
...
snap := ps.MetricSnapshot()
if snap.ActiveSubscriptions != 1 {
t.Errorf("ActiveSubscriptions = %d, want 1 (idempotent consolidated pattern)", snap.ActiveSubscriptions)
}
}
...
t.Errorf("ActiveSubscriptions = %d, want 2 (consolidated pattern for 2 channels)", snap.ActiveSubscriptions)
}
}
