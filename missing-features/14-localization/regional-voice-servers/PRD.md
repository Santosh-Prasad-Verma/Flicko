...
> **One-line:** Auto-pick the lowest-latency LiveKit region per call so voice/video feels instant globally.
...
Today Flicko's LiveKit cluster runs in a single region (US-East). For a Tokyo user joining a stage, audio round-trips ~250ms each way; mouth-to-ear delay exceeds 500ms — perceived as "broken" by anyone who's used Discord (~80ms regional) or Zoom (~120ms regional). The fix is multi-region LiveKit deployments with smart selection per call.
...
**Goals**
- Deploy LiveKit nodes in five regions: NA-East (us-east), NA-West (us-west), EU (eu-west), APAC (ap-southeast), LATAM (sa-east), SEA (ap-south).
...
- LiveKit room federation when participants span regions — so a Tokyo user and a New York user can land in their respective edges and the federation bridges them.
...
- Edge MCU/SFU optimization beyond LiveKit's defaults.
- Hardware-level codec choice — LiveKit handles Opus/VP9 already.
- Self-hosted TURN beyond what LiveKit ships.
- Latency optimization for non-voice (chat, push) — they don't need it.

## 4. Scope (v1)

- [ ] Deploy LiveKit clusters in 6 regions (one shared infra repo `infra/livekit/<region>/`).
...
- [ ] LiveKit room creation API accepts region hint or uses score.
...
|--------|--------|-------------|
| Median voice p50 latency | <100ms global | LiveKit server metrics |
...
| Cost increase | <2x baseline | infra invoices (LiveKit Cloud pricing) |

## 6. Open Questions / Risks

- LiveKit Cloud (managed) charges per minute per participant; multi-region adds federation cost. Need pricing model. Self-host on Hetzner / Fly.io edge nodes likely cheaper at scale.
...
- TURN relay fallback: when P2P fails, we still need a relay near the user — we use LiveKit's built-in TURN per region.
...
| Teams | Anycast routing | We use BGP-anycast on LiveKit edges if needed |
| Element Call (Matrix) | LiveKit-based, federation early | We outperform with smarter scoring |
...
- **LiveKit Cloud** has multi-region; cost scales per minute.
- Alternative: self-hosted on **Hetzner** (FSN, NBG, ASH, HKG, SIN, SAO regions available) with **Coturn** — cheap but ops-heavy.
- v1 plan: LiveKit Cloud Free (~10 minutes audio/video bundled), upgrade to Build plan ($50/mo + usage) before public launch.
